...
def _select_element(user):
    return next(iter(user.find(kind=_ui.select).elements))
...
async def test_conn_click_renders_classified_text(user: User, tmp_path, monkeypatch):
    ...
    with (
        ...
        items[0]._handle_event(
            {"listener_id": next(iter(items[0]._event_listeners)), "args": {}}
        )
...
async def test_clear_cache_button_wipes_directory(user: User, tmp_path, monkeypatch):
    ...
    assert not (tmp_path / ".tcptrace").exists() or not any(
        (tmp_path / ".tcptrace").iterdir()
    )
...
def test_build_xpl_zip_packs_xpl_files(tmp_path):
    ...
    from tcptrace_ng.app import build_xpl_zip
...
async def test_pcap_switch_clears_analysis_state(
    ...
    rows_b = [
        ConnRow(n=9, host_a="x:1", host_b="y:2", raw_line="  9: x:1 - y:2 (a2b)")
    ]

    def fake_list(pcap, timeout=60.0):
        return rows_a if pcap.name == "a.pcap" else rows_b

    with patch.object(app_mod, "list_connections", side_effect=fake_list):
        app_mod.build_page()
        await user.open("/")
        ...
        app_mod.state.analyses = {
            1: AnalyzeResult(details_text="prior", xpl_files=[])
        }
        ...
        assert [r.n for r in app_mod.state.listing] == [9]


async def test_on_pick_falls_back_to_pcap_conversion(
    user: User, tmp_path, monkeypatch
):
    ...
    with (
        ...
        assert [r.n for r in app_mod.state.listing] == [1]


async def test_on_pick_surfaces_error_when_conversion_also_fails(
    user: User, tmp_path, monkeypatch
):
